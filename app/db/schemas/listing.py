# ============================================================
# schemas/listing.py — Pydantic I/O contracts for listing ops
# ============================================================
...
# ── Requests (what callers pass IN) ──────────────────────────

class UpsertListingRequest(BaseModel):
    """Validated input for inserting or updating a scraped listing."""
...
class GetNotifiedUIDsRequest(BaseModel):
    """Fetch all UIDs that have already been notified (for Redis warm-up)."""
    pass   # no parameters — always returns the full set


class ListingStatsRequest(BaseModel):
    """Request aggregate price stats, optionally scoped to a source."""
...
# ── Responses (what callers get BACK) ────────────────────────

class UpsertListingResponse(BaseModel):
    """Result of an upsert operation."""

    listing_db_id: int
    is_new:        bool   # True = never seen before, False = updated existing


class PriceStatsRow(BaseModel):
    """One row of the price stats query."""
