...
class UserService:
    ...
    async def save_filter_state(self, chat_id: str, filt: ApartmentFilter, paused: bool) -> None:
        user = await self.repo.get_user_with_filter(chat_id)
    ...
    def convert_to_domain(db_filter: Filter) -> ApartmentFilter:
        return ApartmentFilter(
            min_price=float(db_filter.min_price) if db_filter.min_price is not None else None,
            max_price=float(db_filter.max_price) if db_filter.max_price is not None else None,
