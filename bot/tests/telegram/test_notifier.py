...
class TestSendApartment:
    ...
    @pytest.mark.asyncio
    async def test_attaches_link_button(self, notifier, bot):
        apt = make_apartment(url="https://example.com/apt/123")
        apt.image_url = None
        await notifier.send_apartment(123, apt, lang="en")
        _, kwargs = bot.send_message.call_args
        markup = kwargs.get("reply_markup")
        assert markup.inline_keyboard[0][0].text == "Link"
        assert markup.inline_keyboard[0][0].url == "https://example.com/apt/123"
